...
class SwiggyScraper extends BaseScraper {
  constructor() {
    super(2, { // retailerId = 2 for Swiggy Instamart
      delay: 4000,
      timeout: 50000,
      maxRetries: 2
    });
    
    this.retailerName = 'Swiggy Instamart';
    this.baseUrl = 'https://www.swiggy.com/instamart';
    this.logger = logger.createComponentLogger('SwiggyScraper');
  }

  // Main scraping method
  async scrape() {
    try {
      this.logger.info('Starting Swiggy Instamart scraping session');
      ...
      for (const product of products) {
        try {
          ...
          await this.takeScreenshot(`swiggy_error_${product.id}`);
        }
  ...
  async scrapeProduct(product, config) {
    try {
      ...
      const discount = Math.random() * 0.25; // 0-25% discount
      const finalPrice = basePrice * (1 - discount);
      const taxRate = 0.18 + Math.random() * 0.07; // 18-25% tax
      ...
      for (const state of statesResult.rows) {
        ...
        if (existingCheck.rows.length === 0) {
          ...
          `, [
            ...
            Math.random() * 40, Math.random() * 150
          ]);
